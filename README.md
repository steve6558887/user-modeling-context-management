...
用户画像包含两个时间尺度：`current_status` 覆盖近三个月轨迹，`最近活动` 覆盖近两天状态。

这两层的必要性在于：当用户在没有任何上下文铺垫的情况下直接表达想法和感受时，LLM 可以基于这两层信息理解用户"为什么现在说这个"——无需用户每次都从头解释背景。同时，这个双层窗口让模型拥有全局视角，能看见用户在 prompt 字面意思之外真正在关心的问题。
